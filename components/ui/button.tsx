...
/**
 * Цель нажатия: 52 px у среднего размера и 60 px у большого — с запасом над
 * минимальными 44 px, потому что запас и есть то, что отличает «попал» от
 * «почти попал» на ходу. Малый размер равен ровно 44 px и ниже минимума уже
 * не опускается.
 *
 * Высоты берутся из токенов `--control-*`: их делят кнопка и поле ввода, и
 * именно поэтому кнопка с полем в одном ряду совпадают по высоте сами, без
 * подгонки на месте.
 *
 * `icon` — квадрат под одну иконку без подписи: стрелки листания, закрытие,
 * действие в строке. Он квадратный по той же средней ступени.
 */
const SIZES: Record<ButtonSize, string> = {
  sm: "min-h-[var(--control-sm)] px-4 text-sm",
  md: "min-h-[var(--control-md)] px-5 text-sm",
  lg: "min-h-[var(--control-lg)] px-7 text-base",
  icon: "size-[var(--control-md)] shrink-0 p-0",
};
